...
export const SignupPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      window.location.hash = '#/analyzer'; // Redirect to analyzer page
    } catch (err: any) {
      ...
      <div className="max-w-md w-full space-y-8 p-8 sm:p-10 bg-slate-800 shadow-2xl rounded-xl border border-slate-700/80 backdrop-blur-sm bg-opacity-80">
        <div>
          <h2 className="mt-6 text-center text-4xl font-display font-extrabold text-white">
            Create an account with <span className="text-purple-400">{APP_NAME}</span>
          </h2>
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
          ...
          <div className="space-y-5 rounded-md shadow-sm">
            <div>
              ...
              <input
                ...
                autoComplete="name"
                required
                className="appearance-none relative block w-full px-4 py-3.5 border border-slate-700 bg-slate-700/60 text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-purple-500 sm:text-sm rounded-lg transition-shadow duration-150 shadow-sm hover:shadow-md focus:shadow-lg"
            ...
            <div>
              <label htmlFor="email-address-signup" className="block text-sm font-medium text-slate-300 mb-1.5">
                Email address
              </label>
              <input
            ...
            <div>
              ...
              <input
                ...
                required
                className="appearance-none relative block w-full px-4 py-3.5 border border-slate-700 bg-slate-700/60 text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-purple-500 sm:text-sm rounded-lg transition-shadow duration-150 shadow-sm hover:shadow-md focus:shadow-lg"
                placeholder="Create a password (any for demo)"
                value={password}
          ...
          <div>
            ...
            >
              {effectiveIsLoading ? <LoadingSpinner size="sm" /> : 'Create Account'}
            </button>
          </div>
           <p className="mt-5 text-center text-xs text-slate-500">
            (Demo: any valid name, email format, and any password will work)
          </p>
        </form>
      </div>
    </div>
